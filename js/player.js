...
/** identifies this presentation to be in presenter mode.
 *  in presenter mode message events will be forwared to the player rather then to the player's controller. 
 * 
 *  @default
 */
...
window.addEventListener("message", evt => onMessage(evt), false);
...
function onMessage(evt){
    switch (evt.data.name) {
        ...
        default: {
            ...
            var receiver = isPresenterMode ? sozi.player : sozi.uicontroller;
...
window.addEventListener("load", () => {
    ...
    const uicontroller = new UIController(player, false);

    Media.init(player);
    FrameList.init(player);
    FrameNumber.init(player);
    FrameURL.init(player);
    TouchGestures.init(uicontroller, player, presentation);
